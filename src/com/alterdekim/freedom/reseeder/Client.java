            ...
            while ((line = scanner.nextLine()) != null) {
                try {
                    ...
                    if (jsonObject.get("act").toString().equals("login")) {
                        String password = ECC.ECCDecode(jsonObject.get("password").toString(), Settings.rsaKeyPair.getPrivateKey());
                        ...
                        for (Client c : socketClients) {
                            if (ECC.SHA256(c.public_key).equals(dest)) {
                                ...
                                if (socketTunnels.size() > 0) {
                                    ...
                                    String hash = ECC.SHA256(String.valueOf(Math.random()));
                                    JSONObject r = new JSONObject();
                                    r.put("act", "tunnel_created");
                                    r.put("ip", ip_t);
                                    r.put("uuid_hash", hash);
                                    r.put("port", Settings.tunnelPortIn);
                                    write(r.toString());
                                    r = new JSONObject();
